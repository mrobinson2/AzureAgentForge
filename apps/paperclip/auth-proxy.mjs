...
// ── iMessage webhook (edge) — OPTIONAL ─────────────────────────────────
// When IMESSAGE_WEBHOOK_SECRET is set, the proxy exposes
// POST /api/webhooks/imessage that accepts iMessageBridge webhook payloads from
// the edge host edge node, validates the shared secret, and creates a
// PaperClip issue assigned to the configured agent (typically Intake).
//
...
// ── Voice-provider end-of-call webhook (voice agent) — OPTIONAL ──────────
// When VOICE_WEBHOOK_SIGNING_SECRET is set, the proxy exposes
// POST /api/webhooks/voice/call-ended that accepts voice-provider end-of-call payloads,
// validates the shared bearer token, and creates a PaperClip issue assigned
// to the configured agent (typically the Business agent) containing
// the structured intake-assessment captured during the call.
//
// When VOICE_WEBHOOK_SIGNING_SECRET is unset (default), the route returns 503
// with a hint listing the env vars to set.
//
// The expected POST body shape is the voice provider end-of-call payload, documented
// inline at the handler below.
const VOICE_WEBHOOK_SIGNING_SECRET = process.env.VOICE_WEBHOOK_SIGNING_SECRET || "";
const VOICE_HANDOFF_AGENT_ID = process.env.VOICE_HANDOFF_AGENT_ID || "";
const VOICE_HANDOFF_COMPANY_ID = process.env.VOICE_HANDOFF_COMPANY_ID || "";
const VOICE_BRIDGE_MARKER = "[voice-agent]";
...
// are neutralised inside the content so it cannot coder an early close.
...
const KNOWN_AGENTS = {
  orchestrator: "Orchestrator (Orchestrator)", strategy: "Strategy (Strategic Commander)",
  planner: "Planner (Planner)", coder: "Coder (Application Coder)",
  infrastructure: "Infrastructure (Infrastructure)", curator: "Curator (Librarian/RAG)",
  researcher: "Researcher (Research)", coach: "Coach (Career Coach)",
  business: "Business (Business Strategy)", psychology: "Psychology (Psychology)",
  qa: "QA (QA)", security: "Security (Security)", "cost-guardian": "CostGuardian (Cost Guardian)",
};
...
/**
 * PUT /api/skills-agents — update agent assignments for a skill.
 * Body: { "skillName": "gws-cli", "agents": ["orchestrator", "curator"] }
...
async function handleRequest(clientReq, clientRes) {
  ...
  if (clientReq.url === "/api/auth/proxy-health") {
    ...
    clientRes.end(JSON.stringify({
      ...
      voice_webhook_enabled: !!VOICE_WEBHOOK_SIGNING_SECRET,
    }));
    return;
  }
  ...
  // ── /api/webhooks/imessage — edge inbound bridge (optional) ─────────
  // iMessageBridge → here. Accepts a `new-message` event, validates the shared
  ...
  if (clientReq.url === "/api/webhooks/imessage" && clientReq.method === "POST") {
    ...
    // Read + parse body (iMessageBridge payloads are small JSON, ~1-5KB)
    const chunks = [];
    let bodySize = 0;
    ...
    // iMessageBridge event shape: { type: "new-message", data: { guid, text,
  ...
  // ── /api/webhooks/voice/call-ended — voice agent (optional) ────
  // the voice provider → here at end of every intake assessment call. Validates the shared
  // signing secret, transforms the voice provider's payload into our assessment_record
  // shape, creates a PaperClip issue assigned to the configured handoff
  // agent (typically Business). When VOICE_WEBHOOK_SIGNING_SECRET is unset, the
  // route returns 503 with a hint — supports the Azure-only fork path with
  // no voice-provider account dependency.
  if (clientReq.url === "/api/webhooks/voice/call-ended" && clientReq.method === "POST") {
    if (!VOICE_WEBHOOK_SIGNING_SECRET || !VOICE_HANDOFF_AGENT_ID || !VOICE_HANDOFF_COMPANY_ID) {
      clientRes.writeHead(503, { "Content-Type": "application/json" });
      clientRes.end(JSON.stringify({
        error: "voice webhook disabled",
        hint: "Set VOICE_WEBHOOK_SIGNING_SECRET, VOICE_HANDOFF_AGENT_ID, and VOICE_HANDOFF_COMPANY_ID env vars to enable the voice webhook.",
      }));
      return;
    }
    if (!hasBearer) {
    ...
    const provided = Buffer.from(authHeader.slice(7).trim());
    const expected = Buffer.from(VOICE_WEBHOOK_SIGNING_SECRET);
    if (provided.length !== expected.length || !timingSafeEqual(provided, expected)) {
      clientRes.writeHead(403, { "Content-Type": "application/json" });
      clientRes.end(JSON.stringify({ error: "Invalid webhook signing secret" }));
      return;
    }
    // Read + parse body (voice-provider payloads typically include transcript + summary;
    ...
    // voice webhook shape:
    //   { skill, vertical_pack, call: { voice_call_id, duration_seconds,
    ...
    // hung_up_during_preamble / error don't generate Business issues.
    const skill = payload.skill || "intake-assessment";
    ...
    const businessSnippet = (captured.business_description || captured["company.description"] || "(unknown business)")
      .toString().slice(0, 60);
    const title = `intake-assessment from ${businessSnippet}`.slice(0, 200);
    const verticalLabel = verticalPack ? `vertical:${verticalPack}` : "vertical:generic";
    // Build the description payload — full structured JSON for Business.
    const description = `${VOICE_BRIDGE_MARKER} caller=${callerPhone} duration=${durationSec}s skill=${skill} disposition=${disposition}\n\n${fenceUntrustedContent(JSON.stringify(payload, null, 2), "voice-call")}`;
    const issuePayload = {
      title,
      description,
      assigneeAgentId: VOICE_HANDOFF_AGENT_ID,
      status: "todo",
      labels: ["intake-assessment", `skill:${skill}`, verticalLabel, `disposition:${disposition}`],
    };
    const ok = await new Promise((resolve) => {
      const req = httpRequest({
        host: BACKEND_HOST,
        port: BACKEND_PORT,
        path: `/api/companies/${VOICE_HANDOFF_COMPANY_ID}/issues`,
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-Automation-Sub": "voice-agent-webhook",
          "Origin": PUBLIC_URL,
        },
      }, (res) => {
        let body = "";
    ...
    clientRes.end(JSON.stringify({
      ...
      created: true,
      identifier: ok.identifier,
      handoff_agent_id: VOICE_HANDOFF_AGENT_ID,
    }));
    return;
  }
