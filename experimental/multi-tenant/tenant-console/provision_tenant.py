#!/usr/bin/env python3
"""
Tenant Provisioning Script (early reference executor).

Kept as a reference only: it predates governed memory, per-run cost envelopes,
and the current PaperClip API surface. The maintained executor lives in
``src/tenantconsole/`` — prefer that. See DESIGN.md for the reuse rationale.

Usage: python provision_tenant.py --slug acme --name "Acme Inc." --email ops@example.com
"""
...
def provision_tenant(api_base: str, slug: str, display_name: str, email: str,
    ...
    try:
        ...
        print(f"   Vault Path: {tenant['obsidian_vault_path']}")
        return tenant
...
def main():
    ...
    parser.add_argument("--orchestrator", action="store_true", default=True, 
                       help="Enable Orchestrator vault")
