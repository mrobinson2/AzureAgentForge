...
# Router-compatible shape, ported from upstream private deployment incident
# learnings (2026-07): Hermes v0.18.x sends x-api-key — NOT "Authorization:
# Bearer" — for the custom provider when api_mode is anthropic_messages, so
# every agent model call died 401 against the router's fail-closed bearer
# auth. The proven-good shape is plain OpenAI chat-completions with api_key
# set (the OpenAI SDK sends Bearer auth the router accepts). api_mode is
# pinned explicitly: without it Hermes v0.18.x still picks the /v1/messages
# transport for tool-heavy runs and re-triggers the 401.
# TRADE-OFF: anthropic_messages (and its cache_control prompt-cache injection
# for Claude tiers) stays OFF until the router also accepts x-api-key auth.
# Canonical copy: apps/paperclip/write-hermes-config.sh (unit-tested via
# tests/paperclip/hermes-config-template.test.mjs); keep the shapes in sync.
...
if [ -z "${HERMES_ROUTER_API_KEY}" ]; then
  echo "[entrypoint] WARNING: ROUTER_API_KEY/OPENAI_API_KEY unset — Hermes agent runs will 401 against the model router (fail-closed)." >&2
fi
...
# using OpenAI chat-completions + Bearer auth (see entrypoint comments for the
# anthropic_messages/x-api-key incident that mandates this shape).
model:
  provider: custom
