...
|---|---|---|---|
| **Auth-proxy (recommended for laptops)** | `MEMORY_API_BASE_URL`, `MEMORY_API_TOKEN` | `Authorization: Bearer <JWT>` (path prefix `/api`) | A control-plane / auth-proxy host is exposed publicly |
...
- **Auth-proxy**: if a public control-plane host fronts the governor, set
