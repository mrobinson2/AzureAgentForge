...
in the private AzureAgentForge platform repo. `scripts/build-and-push.sh` therefore marks the three as
...
to the private AzureAgentForge repo** and **no manual file copying**.
...
no private AzureAgentForge references in committed `apps/` files · gitleaks + trufflehog pass · custom
...
| `apps/paperclip/patch-plugin-secrets-handler.mjs` | paperclip L85 | **embeds verbatim PaperClip code** (§5 attribution) + `[AAF PATCH]` marker (§4 sanitize) |
...
- [ ] No private AzureAgentForge references appear in committed `apps/` (or `build/skills/`) files.
...
6. Sanitize files locally (strip internal org tokens, hostnames, IDs, paths, tokens).
