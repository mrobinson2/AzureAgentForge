...
OpenClaw as an alternative agent runtime. A voice track: shared infrastructure (streaming STT, low-latency TTS, VAD and barge-in, a persona overlay) that stays provider-agnostic across Microsoft Voice Live and Deepgram/ElevenLabs, delivered over three surfaces (Discord voice, a web widget, and a Twilio phone line with a PIN gate, consent, and recording retention). Discord as a control plane: role-gated slash-command operations, in-channel delegation (plan → execute → result), and an audit feed. Complete multi-tenant implementation (the [`experimental/multi-tenant/`](experimental/multi-tenant/) design), including multiple human users per tenant with per-user identity and RBAC. Wiring the v1.3 sandbox seam to an Azure Container Apps dynamic-sessions provider. Human-in-the-loop approval of agent actions and outputs, beyond the infrastructure destroy gate. User-defined scheduled agent routines. Synthetic dogfooding: scheduled canary conversations across channels that alert on repeated failure. A skills manager, plus artifacts and work products. More chat surfaces (Slack, WhatsApp, a web widget). The rest of the observability pipeline: correlation-id threading, per-agent metric counters, an SLO dashboard, SLO burn-rate alerts, and a `gen_ai.usage` cost metric, building on the v1.3 spans and the v1.2 alert rules and workbook. Cost governance: a daily cost rollup and per-user budget caps on top of the per-tier daily caps. Private enterprise RAG with Azure AI Search, Microsoft Foundry Agent Service alignment, and Microsoft 365 / Agent 365 publishing.
