...
OpenClaw as an alternative agent runtime. A voice interface: Microsoft Voice Live, with a provider-agnostic low-latency STT/TTS fallback. Complete multi-tenant implementation (the [`experimental/multi-tenant/`](experimental/multi-tenant/) design), including multiple human users per tenant with per-user identity and RBAC. Wiring the v1.3 sandbox seam to an Azure Container Apps dynamic-sessions provider. Human-in-the-loop approval of agent actions and outputs, beyond the infrastructure destroy gate. User-defined scheduled agent routines. A skills manager, and artifacts and work products. More chat surfaces (Slack, WhatsApp, a web widget). The rest of the observability pipeline: SLO burn-rate alerts and a `gen_ai.usage` cost metric, building on the v1.3 spans and the v1.2 alert rules and workbook. Private enterprise RAG with Azure AI Search, Microsoft Foundry Agent Service alignment, and Microsoft 365 / Agent 365 publishing.
