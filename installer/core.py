...
# Memory peer ids (agent slugs, alias keys, alias targets). Deliberately narrow:
# these are rendered into HCL and compared against peers on the write path, so
# anything exotic is more likely a typo than an intent.
...
class DeployConfig:
    ...
    # Memory peer identity (v1.8.1). Both optional and empty-by-default, which
    # is the same posture the Terraform variables take: no roster declared means
    # nothing is called a stray. The SELF-HOSTED path takes these as
    # HONCHO_AGENT_PEER_IDS / HONCHO_PEER_ALIASES in deploy/mac-site/.env — same
    # names, same semantics, different transport — so a roster is portable
    # between the two. docs/design/memory-system.md §18.
    agent_peer_ids: str = ""
    peer_aliases: str = ""
    ...
    def validate(self) -> list[str]:
        ...
        for oid in self.keyvault_admin_object_ids:
            if not _SUBSCRIPTION_RE.match(oid or ""):
                ...
                break
        for slug in _split_csv(self.agent_peer_ids):
            if not _PEER_ID_RE.match(slug):
                errs.append(f"agent_peer_ids entry {slug!r} is not a valid peer id")
                break
        for pair in _split_csv(self.peer_aliases):
            alias, sep, canonical = pair.partition("=")
            if not sep or not _PEER_ID_RE.match(alias) or not _PEER_ID_RE.match(canonical):
                errs.append(
                    f"peer_aliases entry {pair!r} must be alias=canonical "
                    "(e.g. \"operator=user\")"
                )
                break
        return errs
...
def _split_csv(value: str) -> list[str]:
    """Split a comma-separated input, dropping blanks. Matches how the governor
    parses the same strings, so what validates here is what resolves there."""
...
def render_tfvars(cfg: DeployConfig) -> str:
    ...
    # Normalized (whitespace stripped, blanks dropped) so the value the governor
    # parses is byte-identical to the one shown back to the operator.
    if _split_csv(cfg.agent_peer_ids):
        pairs.append(("honcho_agent_peer_ids",
                      _hcl_str(",".join(_split_csv(cfg.agent_peer_ids)))))
    if _split_csv(cfg.peer_aliases):
        pairs.append(("honcho_peer_aliases",
                      _hcl_str(",".join(_split_csv(cfg.peer_aliases)))))
